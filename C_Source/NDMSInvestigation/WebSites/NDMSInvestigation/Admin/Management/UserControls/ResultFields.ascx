...
<asp:FormView ID="FormView1" runat="server">
	<ItemTemplate>
        ...
        <td>
			...
			<tr>
        <td class="literal"><asp:Label ID="lbldataUpdateDay" runat="server" Text="Update Day:" AssociatedControlID="dataUpdateDay" /></td>
        <td>
					<asp:TextBox runat="server" ID="dataUpdateDay" Text='<%# Bind("UpdateDay", "{0:d}") %>' MaxLength="10"></asp:TextBox><asp:ImageButton ID="cal_dataUpdateDay" runat="server" SkinID="CalendarImageButton" OnClientClick="javascript:showCalendarControl(this.previousSibling);return false;" />
				</td>
			</tr>
			<tr>
        ...
        <td>
					<data:EntityDropDownList runat="server" ID="dataGroupId" DataSourceID="GroupIdQuestionGroupDataSource" DataTextField="GroupName" DataValueField="GroupId" SelectedValue='<%# Bind("GroupId") %>' AppendNullItem="true" Required="true" NullItemText="< Please Choose ...>" ErrorText="Required" />
					<data:QuestionGroupDataSource ID="GroupIdQuestionGroupDataSource" runat="server" SelectMethod="GetAll"  />
				</td>
			</tr>
