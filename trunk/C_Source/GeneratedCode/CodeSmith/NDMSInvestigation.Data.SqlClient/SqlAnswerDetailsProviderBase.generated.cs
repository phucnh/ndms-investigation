...
/*
	...
	Important: Do not modify this file. Edit the file SqlAnswerDetailsProvider.cs instead.
*/
...
namespace NDMSInvestigation.Data.SqlClient
{
	...
	///</summary>
	public abstract partial class SqlAnswerDetailsProviderBase : AnswerDetailsProviderBase
	{
		#region Declarations
		...
		/// <summary>
		/// Creates a new <see cref="SqlAnswerDetailsProviderBase"/> instance.
		/// </summary>
		public SqlAnswerDetailsProviderBase()
		{
		}
	...
	public SqlAnswerDetailsProviderBase(string connectionString, bool useStoredProcedure, string providerInvariantName)
	{
    ...
    public string ProviderInvariantName
    {
		...
		{
			SqlDatabase database = new SqlDatabase(this._connectionString);
			DbCommand commandWrapper = StoredProcedureProvider.GetCommandWrapper(database, "dbo.AnswerDetails_Delete", _useStoredProcedure);
			...
			{
				string entityKey = EntityLocator.ConstructKeyFromPkItems(typeof(AnswerDetails)
					,_answerId);
		...
		public override TList<AnswerDetails> Find(TransactionManager transactionManager, string whereClause, int start, int pageLength, out int count)
		{
			count = -1;
			if (whereClause.IndexOf(";") > -1)
				return new TList<AnswerDetails>();
	
			SqlDatabase database = new SqlDatabase(this._connectionString);
			DbCommand commandWrapper = StoredProcedureProvider.GetCommandWrapper(database, "dbo.AnswerDetails_Find", _useStoredProcedure);
		...
		database.AddInParameter(commandWrapper, "@UpdateBy", DbType.String, DBNull.Value);
			...
			{
				if (clause.Trim().StartsWith("answerid ") || clause.Trim().StartsWith("answerid="))
				{
					database.SetParameterValue(commandWrapper, "@AnswerId", 
						clause.Trim().Remove(0,8).Trim().TrimStart(equalSign).Trim().Trim(singleQuote));
					continue;
				}
				if (clause.Trim().StartsWith("answercontent ") || clause.Trim().StartsWith("answercontent="))
				{
					database.SetParameterValue(commandWrapper, "@AnswerContent", 
						clause.Trim().Remove(0,13).Trim().TrimStart(equalSign).Trim().Trim(singleQuote));
					continue;
				}
				if (clause.Trim().StartsWith("answermark ") || clause.Trim().StartsWith("answermark="))
				{
					database.SetParameterValue(commandWrapper, "@AnswerMark", 
						clause.Trim().Remove(0,10).Trim().TrimStart(equalSign).Trim().Trim(singleQuote));
					continue;
				}
				if (clause.Trim().StartsWith("answerdescription ") || clause.Trim().StartsWith("answerdescription="))
				{
		...
		/// <param name="count">out. The number of rows that match this query.</param>
		/// <returns>Returns a typed collection of NDMSInvestigation.Entities.AnswerDetails objects.</returns>
		public override TList<AnswerDetails> Find(TransactionManager transactionManager, IFilterParameterCollection parameters, string orderBy, int start, int pageLength, out int count)
		{
			...
			SqlDatabase database = new SqlDatabase(this._connectionString);
			DbCommand commandWrapper = StoredProcedureProvider.GetCommandWrapper(database, "dbo.AnswerDetails_Find_Dynamic", typeof(AnswerDetailsColumn), filter, orderBy, start, pageLength);
        ...
        /// <exception cref="System.Data.DataException">The <paramref name="transactionManager"/> is not open.</exception>
        /// <exception cref="System.Data.Common.DbException">The command could not be executed.</exception>
		public override TList<AnswerDetails> GetAll(TransactionManager transactionManager, int start, int pageLength, out int count)
		{
			SqlDatabase database = new SqlDatabase(this._connectionString);
			DbCommand commandWrapper = StoredProcedureProvider.GetCommandWrapper(database, "dbo.AnswerDetails_Get_List", _useStoredProcedure);
		...
		/// <remarks></remarks>
		/// <returns>Returns a typed collection of NDMSInvestigation.Entities.AnswerDetails objects.</returns>
		public override TList<AnswerDetails> GetPaged(TransactionManager transactionManager, string whereClause, string orderBy, int start, int pageLength, out int count)
		{
			SqlDatabase database = new SqlDatabase(this._connectionString);
			DbCommand commandWrapper = StoredProcedureProvider.GetCommandWrapper(database, "dbo.AnswerDetails_GetPaged", _useStoredProcedure);
		...
		{
			SqlDatabase database = new SqlDatabase(this._connectionString);
			DbCommand commandWrapper = StoredProcedureProvider.GetCommandWrapper(database, "dbo.AnswerDetails_GetByAnswerId", _useStoredProcedure);
			
				database.AddInParameter(commandWrapper, "@AnswerId", DbType.Int32, _answerId);
			
			IDataReader reader = null;
			TList<AnswerDetails> tmp = new TList<AnswerDetails>();
			try
			{
				//Provider Data Requesting Command Event
				OnDataRequesting(new CommandEventArgs(commandWrapper, "GetByAnswerId", tmp)); 
				...
				//Provider Data Requested Command Event
				OnDataRequested(new CommandEventArgs(commandWrapper, "GetByAnswerId", tmp));
			}
			finally 
			{
				if (reader != null) 
		...
		/// </remarks>	
		public override void BulkInsert(TransactionManager transactionManager, TList<NDMSInvestigation.Entities.AnswerDetails> entities)
		{
			...
			bulkCopy.DestinationTableName = "AnswerDetails";
			...
			foreach(NDMSInvestigation.Entities.AnswerDetails entity in entities)
			{
				...
				DataRow row = dataTable.NewRow();
					...
					row["AnswerId"] = entity.AnswerId;
							
				
					row["AnswerContent"] = entity.AnswerContent;
							
				
					row["AnswerMark"] = entity.AnswerMark.HasValue ? (object) entity.AnswerMark  : System.DBNull.Value;
		...
		public override bool Insert(TransactionManager transactionManager, NDMSInvestigation.Entities.AnswerDetails entity)
		{
			...
			object _answerId = database.GetParameterValue(commandWrapper, "@AnswerId");
			entity.AnswerId = (System.Int32)_answerId;
